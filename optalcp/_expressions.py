...
class IntExpr(ModelElement):
    ...
    * Boolean expression `precedes` is also optional. Its value could be
    ...
    def __floordiv__(self, other: IntExpr | int) -> IntExpr:
        ...
        Returns a new :class:`IntExpr` representing the integer quotient of `self` and `other`.

        If either operand has value *absent*, the result is also *absent*.

        **Warning:** the result is truncated *toward zero*, not floored toward negative infinity.
        This differs from Python's built-in `//` operator on `int` values:

        - `-7 // 2` evaluates to `-4` in plain Python (floor).
        - `x // y` with `x = -7, y = 2` in an OptalCP model evaluates to `-3` (truncation).

        The operator name `__floordiv__` follows Python's data-model convention (`//` dispatches to it),
        but OptalCP's integer division always rounds toward zero, matching the behavior of C and C# integer
        division.
...
class BoolExpr(IntExpr):
    ...
    *OptalCP* is using 3-value logic: a boolean expression can be `True`, `False`
    ...
    def __or__(self, other: BoolExpr | bool) -> BoolExpr:
        ...
        **Operator precedence:** In Python, `|` has higher precedence than comparison operators but higher than `and_`/`or_`. Use parentheses when combining with comparisons, or use the method/function form:
