...
class StateFunction(ModelElement):
    # Internal.

    def __init__(self, model: Model, transitions: Iterable[Iterable[int]] | None = None):
        if transitions is not None:
            super().__init__(model, "stateFunction", [_wrap_int_matrix(transitions)])
        else:
            super().__init__(model, "stateFunction", [])

    @staticmethod
    def _wrap(expr: StateFunction) -> _ScalarArgument:
        """Internal: Convert a StateFunction to an argument."""
        if isinstance(expr, StateFunction):  # type: ignore[misc]
            return expr._as_arg()
        raise TypeError(f"Expected StateFunction. Got {type(expr).__name__}")

    def _always_equal(self, interval: IntervalVar, value: int) -> Constraint:
        out_params: list[_Argument] = [self._as_arg(), IntervalVar._wrap(interval), _wrap_int(value)]
        return Constraint(self._model, "alwaysEqual", out_params)
...
class IntervalVar(ModelElement):
    ...
    def optional(self) -> bool:
        ...
        **Note:** This property reflects the presence status in the model
        ...
        """
        assert self._props.get('status') != _PresenceStatus.Absent
