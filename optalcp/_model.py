...
from ._scheduling import IntervalVar, IntStepFunction, SequenceVar, StateFunction
...
class Model:
    ...
    * :meth:`Model.maximize`: maximize an integer expression.
    ...
    def state_function(self, transitions: Iterable[Iterable[int]] | None = None) -> StateFunction:
        # Internal.
        return StateFunction(self, transitions)
    ...
    def _from_dict(self, data: dict[str, Any]) -> None:
        ...
        # Reconstruct variable objects from refs, converting absent→optional with empty domain
        for i, props in enumerate(self._refs):
            func = props.get('func')
            if func in ('intVar', '_intVar'):
                if props.get('status') == _PresenceStatus.Absent:
                    props['status'] = _PresenceStatus.Optional
                    props['min'] = IntVarMax
                    props['max'] = IntVarMin
                if func == 'intVar':
                    self._int_vars.append(IntVar(self, props, i))
            elif func in ('boolVar', '_boolVar'):
                if props.get('status') == _PresenceStatus.Absent:
                    props['status'] = _PresenceStatus.Optional
                    props['min'] = 1
                    props['max'] = 0
            ...
            elif func in ('intervalVar', '_intervalVar'):
                if props.get('status') == _PresenceStatus.Absent:
                    props['status'] = _PresenceStatus.Optional
    ...
    def not_(self, arg: BoolExpr | bool) -> BoolExpr:
        ...
        :param arg: The argument to negate (:class:`BoolExpr` or `bool`).
    ...
    def or_(self, lhs: BoolExpr | bool, rhs: BoolExpr | bool) -> BoolExpr:
        ...
        :param lhs: The left-hand side operand (:class:`BoolExpr` or `bool`).
        :type lhs: BoolExpr | bool
        :param rhs: The right-hand side operand (:class:`BoolExpr` or `bool`).
    ...
    def guard(self, arg: IntExpr | int, absent_value: int = 0) -> IntExpr:
        ...
        :param arg: The argument to guard (:class:`IntExpr` or `int`).
    ...
    def identity(self, lhs: IntExpr | int, rhs: IntExpr | int) -> Constraint:
        ...
        :param lhs: The left-hand side operand (:class:`IntExpr` or `int`).
        :type lhs: IntExpr | int
        :param rhs: The right-hand side operand (:class:`IntExpr` or `int`).
    ...
    def in_range(self, arg: IntExpr | int, lb: int, ub: int) -> BoolExpr:
        ...
        :param arg: The argument to check (:class:`IntExpr` or `int`).
        :type arg: IntExpr | int
    ...
    def abs(self, arg: IntExpr | int) -> IntExpr:
        ...
        :param arg: The argument (:class:`IntExpr` or `int`).
        :type arg: IntExpr | int
        :rtype: IntExpr
        :returns: The resulting integer expression
    ...
    def _selective_disjunction(self, intervals: Iterable[IntervalVar], types: Iterable[int], transitions: Iterable[Iterable[int]] | None = None) -> Constraint:
        out_params: list[_Argument] = [IntervalVar._wrap_list(intervals), _wrap_int_list(types)]
        if transitions is not None:
            out_params.append(_wrap_int_matrix(transitions))
        return Constraint(self, "selectiveDisjunction", out_params)

    def _always_equal(self, func: StateFunction, interval: IntervalVar, value: int) -> Constraint:
        out_params: list[_Argument] = [StateFunction._wrap(func), IntervalVar._wrap(interval), _wrap_int(value)]
        return Constraint(self, "alwaysEqual", out_params)
